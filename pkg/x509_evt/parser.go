...
func parsePublicKey(keyData *publicKeyInfo) (any, error) {
	...
	case oid.Equal(oidSignatureMLDSA44):
		...
		if len(der) != mldsa44.PublicKeySize {
			return nil, errors.New("x509: wrong MLDSA-44 public key size")
		}

		var sizedBuffer [mldsa44.PublicKeySize]byte

		copy(sizedBuffer[:], der)
	...
	case oid.Equal(oidSignatureMLDSA65):
		...
		if len(der) != mldsa65.PublicKeySize {
			return nil, errors.New("x509: wrong MLDSA-65 public key size")
		}

		var sizedBuffer [mldsa65.PublicKeySize]byte
	...
	case oid.Equal(oidSignatureMLDSA87):
		...
		if len(der) != mldsa87.PublicKeySize {
			return nil, errors.New("x509: wrong MLDSA-87 public key size")
		}

		var sizedBuffer [mldsa87.PublicKeySize]byte
		...
		if len(e.Id) == 4 && e.Id[0] == 2 && e.Id[1] == 5 && e.Id[2] == 29 {
			...
			case 33:
				...
				for !val.Empty() {
					...
					issuerOid, _ := newOIDFromDER(issuer)
					subjectOid, _ := newOIDFromDER(subject)
					out.PolicyMappings = append(out.PolicyMappings, x509.PolicyMapping{issuerOid, subjectOid})
				}
			case 54:
				val := cryptobyte.String(e.Value)
